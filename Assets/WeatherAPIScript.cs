...
{
    public GameObject chicagoTextObject;
    public GameObject englandTextObject;
        // add your personal API key after APPID= and before &units=
       string chicagoWeather = "http://api.openweathermap.org/data/2.5/weather?lat=33.39&lon=-104.5&APPID=a63a0b04f2e6522777a363f0a417bfb7&units=imperial";
       string englandWeather = "http://api.openweathermap.org/data/2.5/weather?lat=51.50&lon=-0.12&APPID=a63a0b04f2e6522777a363f0a417bfb7&units=metric";
   ...
   void GetDataFromWeb()
   {
       ...
       StartCoroutine(GetChicagoRequest(chicagoWeather));
       StartCoroutine(GetEnglandRequest(englandWeather));
   }

    IEnumerator GetChicagoRequest(string uri)
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                chicagoTextObject.GetComponent<TextMeshPro>().text = "" + easyTempF.ToString() + "°F\n" + conditions;
            }
        }
    }

        IEnumerator GetEnglandRequest(string uri)
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            // Request and wait for the desired page.
            ...
            else
            {
                ...
                englandTextObject.GetComponent<TextMeshPro>().text = "" + easyTempF.ToString() + "°C\n" + conditions;
            }
        }
    }
}
